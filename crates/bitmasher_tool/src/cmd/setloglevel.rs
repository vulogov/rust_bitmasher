...
pub fn setloglevel(c: &cmd::Cli) {
    let env = Env::default().filter_or("BITMASHER_LOG_LEVEL", "error");
    match c.debug {
        0 => {
        ...
        1 => {
            let env = Env::default()
                .filter_or("BITMASHER_LOG_LEVEL", "bitmasher=info,rust_bitmasher=info");
            env_logger::init_from_env(env);
        ...
        2 => {
            let env = Env::default().filter_or(
                "BITMASHER_LOG_LEVEL",
                "bitmasher=debug,rust_bitmasher=debug",
            );
            env_logger::init_from_env(env);
        ...
        _ => {
            let env = Env::default().filter_or(
                "BITMASHER_LOG_LEVEL",
                "bitmasher=trace,rust_bitmasher=trace",
            );
            env_logger::init_from_env(env);
